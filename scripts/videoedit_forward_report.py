...
name = "case0008_92f_40s_ref0_tight"
...
if "--wait" in sys.argv:
    ...
    while True:
        logs = [(work / f"{name}_{side}.log").read_text(errors="replace") for side in ("reference", "sglang")]
...
report = {"configuration": dict(frames=92, steps=40, reference_frame=0, strict=False,
                                  infer_len=49, overlap=5, seed=42, teacache=False),
          "raw": {}, "encoded": {}, "video_streams": {}}
for window, start, offset in [(0, 1, 0), (1, 5, 48)]:
    ...
    for index in range(start, 49):
        ...
        frames.append(dict(index=offset+index-start, ssim=ssim, mse=mse, mae=mae,
...
for variant, suffix, threshold in [("crop", "_crop_only", .97), ("full", "", .98)]:
    ...
    result["windows"] = {"window0": summarize(result["frames"][:48]),
                         "window1": summarize(result["frames"][48:])}
