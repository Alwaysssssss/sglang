...
- PSNR：越高越接近 BF16；它主要反映像素误差。
- SSIM：越接近 1 越好；它关注亮度、对比度和结构相似度。
- SSIM P05：最差 5% 帧附近的水平，用于避免均值掩盖少量坏帧。
- MAE：归一化 RGB 像素的平均绝对误差，越低越好。
- 当前初筛门槛为 SSIM 均值不低于 `0.95`、P05 不低于 `0.90`。全画面通过；Mask 内 P05 也高于 `0.90`。
