...
For programmatic use (e.g. building sandbox providers), use the Python SDK:
...
async with await Sandbox.create("worker", image="python:3.12", memory=1024, cpus=2) as sb:
    # Run commands
    output = await sb.shell("echo hello")
    print(output.stdout_text, output.exit_code)

    # Filesystem
    await sb.fs.write("/app/data.txt", b"hello")
    content = await sb.fs.read_text("/app/data.txt")
    entries = await sb.fs.list("/app")

    # Binary execution
    output = await sb.exec("python", ["-c", "print(42)"])
```

Key APIs:
- `Sandbox.create(name, image=..., memory=..., cpus=..., ...)` — create a microVM
- `sb.shell(cmd)` / `sb.exec(cmd, args)` — run commands → `ExecOutput` (`.stdout_text`, `.exit_code`)
- `sb.fs.read_text(path)` / `sb.fs.write(path, data)` / `sb.fs.list(path)` — filesystem
- `sb.stop()` / `sb.kill()` / `sb.detach()` — lifecycle
- `Sandbox.get(name)` / `Sandbox.list()` — discover existing sandboxes

For full Python SDK reference, see [references/sdk-python.md](references/sdk-python.md).
...
For SDK usage, see [references/sdk-python.md](references/sdk-python.md), [references/sdk-typescript.md](references/sdk-typescript.md), and [references/sdk-rust.md](references/sdk-rust.md).
