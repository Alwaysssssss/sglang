...
def _interpolate_opencv(
    ...
    opencv_interpolation_map = {
        "nearest": cv2.INTER_NEAREST,
        "linear": cv2.INTER_LINEAR,
        "bilinear": cv2.INTER_LINEAR,
        "bicubic": cv2.INTER_CUBIC,
    }
    assert interpolation in opencv_interpolation_map
    new_h, new_w = size
    img_array_list = [
    ...
    resized_img_array_list = [
        cv2.resize(
            img_array,
            (new_w, new_h),
            interpolation=opencv_interpolation_map[interpolation],
        )
...
class VisionExpertAttention(nn.Module):
    ...
    def forward(
        ...
        has_past_key_value = past_key_value is not None and past_key_value[0] is not None
...
class CogVLMVideoModel(CogVLMPreTrainedModel):
    ...
    def llm_forward(
        ...
        dynamic_cache_seq_length = None
        if (
            past_key_values is not None
            and not hasattr(past_key_values, "__getitem__")
            and hasattr(past_key_values, "__iter__")
        ):
            if hasattr(past_key_values, "get_seq_length"):
                dynamic_cache_seq_length = past_key_values.get_seq_length()
            past_key_values = tuple(past_key_values)

        if past_key_values is not None:
            if dynamic_cache_seq_length is not None:
                past_key_values_length = dynamic_cache_seq_length
